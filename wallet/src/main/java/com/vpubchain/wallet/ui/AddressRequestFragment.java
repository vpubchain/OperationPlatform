...
import com.vpubchain.wallet.ui.dialogs.CreateNewAddressDialog;
import com.vpubchain.wallet.ui.widget.AmountEditView;
import com.vpubchain.wallet.util.QrUtils;
...
import static com.vpubchain.wallet.ExchangeRatesProvider.getRate;
...
public class AddressRequestFragment extends WalletFragment {
    ...
    public void updateView() {
        ...
        if (showAddress != null) {
            ...
            //获取到地址 AbstractAddress
            receiveAddress = account.getReceiveAddress();
            Log.i("receiveAddress",receiveAddress.getId()+"     "+receiveAddress.getType()+"    "+receiveAddress.toString());
        }

        // Don't show previous addresses link if we are showing a specific address如果显示特定地址，则不显示以前的地址链接
