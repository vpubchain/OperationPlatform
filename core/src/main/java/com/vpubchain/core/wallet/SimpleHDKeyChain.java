...
public class SimpleHDKeyChain implements EncryptableKeyChain, KeyBag {
    ...
    /**
     * Returns leaf keys issued by this chain (including lookahead zone but no lookahead threshold)返回此链颁发的叶键（包括先行区域，但没有先行阈值）
     */
    public List<DeterministicKey> getActiveKeys() {
        ...
        for (ECKey key : getKeys(true)) {
            log.error(getKeys(true).size()+"地址个数");
