...
import com.vpubchain.core.wallet.families.nxt.NxtFamilyWalletProtobufSerializer;
...
public class WalletProtobufSerializer {
    ...
    public static Protos.Wallet toProtobuf(Wallet wallet) {
        ...
        for (WalletAccount account : wallet.getAllAccounts()) {
            ...
            } else if (account instanceof EthWalletPocketHD){
                pocketProto = WalletPocketProtobufSerializer.toProtobuf((EthWalletPocketHD) account);
     ...
     *<p>解析给定流中的钱包，使用提供的钱包实例将数据加载到。这主要是
     *用于注册扩展名。协议中的数据将添加到钱包中（如适用），以及
     *覆盖不存在的地方。<p>
     *<p>由于各种原因，钱包可能无法读取，例如无法打开文件、内部数据损坏。
     *数据不一致，钱包扩展被标记为强制性，无法处理等。你应该永远
     *处理@link unreadablewalletexception并以适当的方式向用户传达故障。<p>
     *

     *@引发在各种错误条件下引发的不可读WalletException（请参见说明）。
