...
public class CoinURI implements Serializable {
    ...
    public CoinURI(@Nullable CoinType uriType, String input) throws CoinURIParseException {
        ...
        if (type == null) {
            if (uri.getScheme() != null) {
                try {
                    ...
                    possibleTypes = CoinID.fromUriScheme(scheme);
                    //获取继承coinType的类  如DashMain 覆写的uriScheme   vpub
        ...
        if (!addressToken.isEmpty()) {
            ...
            for (CoinType possibleType : possibleTypes) {
                ...
                } else if (possibleType instanceof BitFamily){
                    try {
     ...
     * 将该值与键相对应，放入地图检查是否重复。这样可以避免地址字段重写等。
     ...
     * @return The {@link com.vpubchain.core.coins.CoinType} of this URI
     */
